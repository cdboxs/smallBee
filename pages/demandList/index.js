      ...
      success: function (res) {
        if (res.data.islogin == 1) {
          ...
          setTimeout(function () {
            wx.hideLoading();
            wx.navigateTo({
              url: '../mdemand/index?select=2' + '&classname=' + e.currentTarget.dataset.classname + '&classid=' + e.currentTarget.dataset.classid,
            })
          }, 500);
        }
      },
  ...
  getMerChantList: function (class_id,postpage){
    ...
    if (class_id) {
      wx.getStorage({
        ...
        success: function (res) {
              wx.request({
                url: app.globalData.APIURL + '/api/News/newsList',
                method: 'POST',
                header: {
